...
{{ printf "%s-cm-%s-%s-%s-fluentbit-cfg-%s" .Values.k8sPrefix .Values.customer .Values.purpose (include "newrelic-logging.fullname" .) .Values.stage }}
{{- end -}}
...
{{- if .Values.global}}
  ...
  {{- else -}}
    {{- .Values.cluster | default (printf "%s-eksc-%s-%s-%s-license-%s" .Values.awsAccount .Values.customer .Values.purpose .Values.stage) -}}
  {{- end -}}
{{- else -}}
    {{- .Values.cluster | default (printf "%s-eksc-%s-%s-%s" .Values.awsAccount .Values.customer .Values.purpose .Values.stage) -}}
{{- end -}}
{{- end -}}
...
{{- if .Values.global }}
  ...
  {{- else -}}
      {{- .Values.customSecretName | default (printf "%s-secret-%s-%s-%s-license-%s" .Values.k8sPrefix .Values.customer .Values.purpose (include "newrelic-logging.fullname" .) .Values.stage) -}}
  {{- end -}}
{{- else -}}
    {{- .Values.customSecretName | default (printf "%s-secret-%s-%s-%s-license-%s" .Values.k8sPrefix .Values.customer .Values.purpose (include "newrelic-logging.fullname" .) .Values.stage) -}}
{{- end -}}
...
{{- if .Values.global }}
  ...
  {{- else -}}
    ...
    {{- else -}}
        {{- .Values.customSecretKey | default "licenseKey" -}}
    {{- end -}}
  {{- end -}}
{{- else -}}
